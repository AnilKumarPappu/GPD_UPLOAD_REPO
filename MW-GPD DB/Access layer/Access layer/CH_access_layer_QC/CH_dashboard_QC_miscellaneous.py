...
## Campaign level
df_fact_performance.filter(col('country_id')=='TW').filter(col('campaign_desc')=='tw_exg_tw-mw-30-extrap11campaign-yt-oct2023_mw_apac_gnm_brand_olv_awareness_bid-adr_1023_youtube').groupBy('country_id','campaign_desc').agg(sum('impressions'),sum('views'),sum('video_fully_played')).display()

# COMMAND ----------

## creative level
df_fact_performance.filter(col('country_id')=='TW').filter(col('campaign_desc')=='tw_exg_tw-mw-30-extrap11campaign-yt-oct2023_mw_apac_gnm_brand_olv_awareness_bid-adr_1023_youtube').join(dim_creative,on= 'creative_id',how= 'inner').filter(col('creative_desc')=='tw_exg_tw-mw-30-extrap11campaign-yt-oct2023_zh_dv36o_yt_bid-adr_demo-only_18-44_extra-gum_v1_video_none_15s_skippable-vtr_na_mars_comb-in-between-moments-affinity (opid-3745613)_imp').agg(sum('impressions'),sum('views'),sum('video_fully_played')).display()

# COMMAND ----------

# creative count
df_fact_performance.filter(col('country_id')=='TW').filter(col('campaign_desc')=='tw_exg_tw-mw-30-extrap11campaign-yt-oct2023_mw_apac_gnm_brand_olv_awareness_bid-adr_1023_youtube').join(dim_creative,on= 'creative_id',how= 'inner').select('creative_desc').dropDuplicates().display()

# COMMAND ----------

df_fact_performance.filter(col('country_id')=='TW').select('campaign_desc').dropDuplicates().display()
