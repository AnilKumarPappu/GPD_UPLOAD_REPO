...
# for now some data for country X year is not coming from source, in future you may consider to check this 
...
fact_creativeX.groupBy("country_id").agg(
    max("date_id"), sum('spend'),sum('clicks'),sum('impressions'),sum('video_3_sec_views'),sum('video_15_sec_views'),sum('video_30_sec_views'),sum('video_completion_25'),sum('video_completion_50'),sum('video_completion_75'),sum('video_fully_played'),sum('creative_quality_score')).display()
