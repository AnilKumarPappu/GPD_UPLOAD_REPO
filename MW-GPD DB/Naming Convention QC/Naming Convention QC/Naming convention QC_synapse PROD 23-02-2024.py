...
############ these values are removed as in the latest file sent by EMC, these are not included '122, 123, 124, 125, 126, 127, 222, 223, 224, 225, 226, 227, 322, 323, 324, 325, 326, 327, 422, 423, 424, 425, 426, 427, 522, 523, 524, 525, 526, 527, 622, 623, 624, 625, 626, 627, 722, 723, 724, 725, 726, 727, 822, 823, 824, 825, 826, 827, 922, 923, 924, 925, 926, 927'
...
##################  removing dv36o from he below list
...
##################  removing DV36O (O is letter oo) from he below list
...
####################### removing dv36o from below list
...
dim_creative23_platform_filtered.groupby('marketregion_code')\
...
# with filtered platforms
dim_creative23_platform_filtered.groupby('marketregion_code','platform_desc')\
    .agg(count('creative_desc').alias("total_count"),\
        count(when(col("naming_convention_flag")==0, True)).alias("following"),\
        count(when(col("naming_convention_flag")==1, True)).alias("not_following")).display()
...
# with filtered platforms
dim_creative23_platform_filtered.groupby('country_desc')\
    .agg(count('creative_desc').alias("total_count"),\
        count(when(col("naming_convention_flag")==0, True)).alias("following"),\
        count(when(col("naming_convention_flag")==1, True)).alias("not_following")).display()
