...
df_fact_performance_synapse.join(dim_campaign_synapse.select('gpd_campaign_id','campaign_desc_free_field'), on = 'gpd_campaign_id', how= 'inner').join(dim_creative_synapse.select('creative_id','creative_campaign_desc_free_field','dimension','creative_desc'), on = 'creative_id', how = 'inner').join(dim_channel_synapse.select('channel_id','platform_desc'), on ='channel_id', how = 'inner' ).join(dim_mediabuy_synapse.select('media_buy_id','placement_type'),on = 'media_buy_id', how = 'inner' ).groupBy('campaign_desc_free_field','country_id','platform_desc','placement_type','dimension','creative_campaign_desc_free_field','creative_desc').agg(sum('views').alias('views'), sum('video_plays').alias('video_plays'),sum('impressions').alias('impressions'),sum('video_completion_25'),sum('video_completion_50'),sum('video_completion_75'),sum('video_fully_played').alias('video_fully_played')).withColumn('VTR', col('views')/col('impressions') ).withColumn('VCR', col('video_fully_played')/col('views')).display()

# COMMAND ----------

# dff.filter(dff['creative_desc'].like('%_______%')).display()
dff.filter(dff['creative_desc'].rlike(r'\_\_\_')).display()


# COMMAND ----------

dff= df_fact_performance_synapse.join(dim_campaign_synapse.select('gpd_campaign_id','campaign_desc_free_field'), on = 'gpd_campaign_id', how= 'inner').join(dim_creative_synapse.select('creative_id','creative_campaign_desc_free_field','dimension','creative_desc'), on = 'creative_id', how = 'inner').join(dim_channel_synapse.select('channel_id','platform_desc'), on ='channel_id', how = 'inner' ).join(dim_mediabuy_synapse.select('media_buy_id','placement_type'),on = 'media_buy_id', how = 'inner' ).groupBy('campaign_desc_free_field','campaign_start_date','campaign_end_date','country_id','platform_desc','placement_type','dimension','creative_campaign_desc_free_field','creative_desc').agg(sum('views').alias('views'), sum('video_plays').alias('video_plays'),sum('impressions').alias('impressions'),sum('video_completion_25'),sum('video_completion_50'),sum('video_completion_75'),sum('video_fully_played').alias('video_fully_played')).withColumn('VTR', col('views')/col('impressions') ).withColumn('VCR', col('video_fully_played')/col('views')).filter(col('country_id')=='US').filter(col('platform_desc')=='DV360-YOUTUBE')
dff.display()
# dff.filter(dff['creative_desc'].like('%_______%')).display()
