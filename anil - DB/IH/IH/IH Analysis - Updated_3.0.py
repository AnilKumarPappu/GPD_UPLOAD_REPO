...
prod_ih_ingoing_3.join(prod_dim_country_3,on='country_id',how='left').join(prod_dim_product_3,on= 'product_id',how= 'left').groupBy('country_desc',
 'product',
 'budget_year',
 'plan_desc',
 'media_type',
...
prod_ih_ingoing_3.join(prod_dim_country_3,on='country_id',how='left').join(prod_dim_product_3,on= 'product_id',how= 'left').filter(col('budget_year').isin('2019','2020','2021','2022')).groupBy('country_desc',
 'product',
 'budget_year',
 'plan_desc',
 'media_type').agg(sum('ingoing_spend_per_day_lc').alias('ingoing_spend_lc'),sum('ingoing_spend_per_day_usd').alias('ingoing_spend_usd')).display()
...
# MAGIC %md
# MAGIC ## USD/Lc ratio for actual and ingoing spend 

# COMMAND ----------

# filter countries and year
countries = ['UZ', 'KR', 'VN' ,'ID']
year = ['2019','2020','2021','2022','2023']
prod_ih_spend_3 = prod_ih_spend_3.select('country_id','budget_year','cost_per_day_lc','cost_per_day_usd').filter(col("country_id").isin(countries)).filter(col("budget_year").isin(year))
prod_ih_spend_2.display()

# COMMAND ----------

prod_ih_spend_3.count()
...
prod_ih_spend_3.groupBy('country_id','budget_year').agg(sum('cost_per_day_lc').alias('cost_per_day_lc'),sum('cost_per_day_usd').alias('cost_per_day_usd')).withColumn('actual_USD/LC_ratio',col('cost_per_day_usd')/col('cost_per_day_lc')).display()
...
countries = ['UZ', 'KR', 'VN' ,'ID']
year = ['2019','2020','2021','2022','2023']
prod_ih_ingoing_3 = prod_ih_ingoing_3.select('country_id','budget_year','ingoing_spend_per_day_lc','ingoing_spend_per_day_usd').filter(col("country_id").isin(countries)).filter(col("budget_year").isin(year))
prod_ih_ingoing_2.display()

# COMMAND ----------

prod_ih_ingoing_2.count()

# COMMAND ----------

prod_ih_ingoing_3.groupBy('country_id','budget_year').agg(sum('ingoing_spend_per_day_lc').alias('ingoing_spend_per_day_lc'),sum('ingoing_spend_per_day_usd').alias('ingoing_spend_per_day_usd')).withColumn('ingoing_USD/LC_ratio',col('ingoing_spend_per_day_usd')/col('ingoing_spend_per_day_lc')).display()
...
dev_dim_product.distinct().count() #714

# COMMAND ----------

#11: No duplicate rows in dim_country
#dev_dim_country.count() #86 #85
dev_dim_country.distinct().count() #86
...
# MAGIC select *
# MAGIC
# MAGIC from prod_global_xseg_srm_data_foundation_mm.gmd_fact_performance
...
spend_new.groupby('country_id','budget_year').agg(sum('ingoing_spend_per_day_lc'),sum('ingoing_spend_per_day_usd')).display()
...
ingoing_new.select('country_id','budget_year', 'plan_name','media_type','start_date','end_date','ingoing_spend_lc').drop_duplicates().groupby('country_id','budget_year').agg(sum('ingoing_spend_lc')).display()
