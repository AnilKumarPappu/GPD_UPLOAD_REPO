...
dim_campaign_syn = jdbc_connection_synapse('mm_test.vw_mw_gpd_dim_campaign')
dim_creative_syn = jdbc_connection_synapse('mm_test.vw_mw_gpd_dim_creative')
dim_mediabuy_syn = jdbc_connection_synapse('mm_test.vw_mw_gpd_dim_mediabuy')
dim_country_syn = jdbc_connection_synapse('mm_test.vw_mw_gpd_dim_country')
df_fact_performance_synapse = jdbc_connection_synapse('mm_test.vw_mw_gpd_fact_performance')
dim_channel_synapse = jdbc_connection_synapse('mm_test.vw_mw_gpd_dim_channel')


# COMMAND ----------

dim_creative_syn.filter(col('creative_desc').isin(['M&Ms_CPM_Image_Pinterest_CPM_18-54_18+DO_image4-AH',"P11 2022_Mars Wrigley M&M's NL EMEA_NL_Amazon_Amazon (Mars)_18-54_Entertainment_Halloween 15s_Hybrid_Hybrid_Hybrid_In-stream (OPID-3315753)_Imp",'M&Ms_Reach_Social Multi_Pinterest_CPM_18-49_LE/DT_Blocks tickle cookie_Jumbo',"Q4 2022_Mars Wrigley M&M's NL EMEA_NL_DBM_DBM-Youtube_M&Ms_Reach_Bumper_YouTube_CPM_18-65+_18+DO_Pre Game_Football_Pre_GAME NED-SEG_Hybrid_Hybrid_Hybrid_In-stream (OPID-3336324)_Imp"])).display()

# COMMAND ----------

dim_country_syn.select('gpd_region_desc').distinct().display()

# COMMAND ----------

print(dim_campaign_syn.count())
print(dim_creative_syn.count())
print(dim_mediabuy_syn.count())

# COMMAND ----------

campaign_NA = dim_campaign_syn.join(dim_country_syn,on = 'country_id',how = 'inner').filter((col('gpd_region_desc') == 'NA') & (col('media_channel') == 'social'))
creative_NA = dim_creative_syn.join(dim_country_syn,on = 'country_id',how = 'left').filter((col('gpd_region_desc') == 'NA')).join(df_fact_performance_synapse.select('creative_id','channel_id').dropDuplicates(),on= 'creative_id',how = 'left').join(dim_channel_synapse.select('channel_id','channel_desc').dropDuplicates(),on = 'channel_id', how = 'left').filter(col('channel_desc')=='Social')
mediabuy_NA = dim_mediabuy_syn.join(dim_country_syn,on = 'country_id',how = 'inner').filter((col('gpd_region_desc') == 'NA') ).join(df_fact_performance_synapse.select('media_buy_id','channel_id').dropDuplicates(),on= 'media_buy_id',how = 'inner').join(dim_channel_synapse.select('channel_id','channel_desc').dropDuplicates(),on = 'channel_id', how = 'inner').filter(col('channel_desc')=='Social')
...
# dim_campaign_synapse_NC = campaign_NA.withColumn(
#     "campaign_NC", when(
...
#         (preproc_udf(col("segment"…preproc_udf(col("region")).isin(campaign_dict['region'])) & 
...
campaign_mismatch = dim_campaign_synapse_NC.filter(col("campaign_NC")!=col("naming_convention_flag"))
campaign_mismatch.display()

# COMMAND ----------

# Following and non following
print(dim_campaign_synapse_NC.filter(col('naming_convention_flag')==0).count())
print(dim_campaign_synapse_NC.filter(col('naming_convention_flag')==1).count())
...
dim_mediabuy_synapse_NC = mediabuy_NA.withColumn(
    "mediabuy_NC", when(
...
# Following and non following
print(dim_mediabuy_synapse_NC.filter(col('naming_convention_flag')==0).count())  # following
print(dim_mediabuy_synapse_NC.filter(col('naming_convention_flag')==1).count()) # not following

# COMMAND ----------

mediabuy_mismatch = dim_mediabuy_synapse_NC.filter(col("mediabuy_NC")!=col("naming_convention_flag"))
...
dim_creative_synapse_NC = creative_NA.withColumn(
    "creative_NC", when(
...
# Following and non following
print(dim_creative_synapse_NC.filter(col('naming_convention_flag')==0).count())  # following
print(dim_creative_synapse_NC.filter(col('naming_convention_flag')==1).count()) # not following

# COMMAND ----------

creative_mismatch = dim_creative_synapse_NC.filter(col("creative_NC")!=col("naming_convention_flag"))
