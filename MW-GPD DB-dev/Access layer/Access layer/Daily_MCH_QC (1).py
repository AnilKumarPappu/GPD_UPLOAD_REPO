...
# df_reach.filter(col('gpd_campaign_id') == 'DATORAMA_BE_1016853217').display()
...
brand = pd.read_excel("/Workspace/Shared/GPD-MW-DEV/Access layer/brand.xlsx", 'new')
...
final_unmatched = merged_df.filter(col('brand_in_access_layer') != col('Brand_mapped'))
final_unmatched = final_unmatched.select(
    "year", "marketregion_code", "country_id", "brand_in_access_layer", "Brand_mapped", "campaign_subproduct", "campaign_desc"
)

final_unmatched.display()
